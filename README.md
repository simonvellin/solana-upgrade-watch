...
- for **Squads v4 multisigs**: the real threshold (approvals required over
  voting members, matching the Squads UI), member count, and **timelock**;
...
Or just clone it. Pure standard-library Python (3.10+), no dependencies.
...
Watch mode polls and alerts on any change (authority rotation, deploy,
new queued proposal):
...
no indexer, and the tool never stores or sends anything anywhere:
...
3. Squads v4 vault PDAs cannot be reversed mathematically, so the multisig
   is identified from the vault's transaction history (fast path), falling
   back to deriving every v4 multisig's vault PDAs locally until one
   matches.
